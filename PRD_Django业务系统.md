...
系统不再使用一个全局混放的 `config.yaml`。后续采用 `storage/task_000001` 这种任务目录作为基本管理单元，每个任务目录自带自己的 `config.yaml`，由 Django 和 MLflow 实验子系统共享。
...
- 创建与管理任务目录，例如 `storage/task_000001`
...
1. Django 接收任务请求
2. 生成或确认当前 `task_id`
3. 确保对应的 `storage/task_000001` 这类任务目录存在
4. 生成 `task_id`
5. 记录任务状态为 `pending`
...
`config.yaml` 放在每个 `storage/task_000001` 这类任务目录内，并由 Django 与 MLflow 共用。
...
- 用户是否可以读取某个 `storage/task_000001`
- 用户是否可以下载某个 `storage/task_000001` 下的结果
- 用户是否可以访问某个 `storage/task_000001` 对应的 MLflow 链接
...
在资源组织上，Django 后续要以 `storage/task_000001` 这类任务目录作为最小管理单位，围绕 `task_id` 来管理数据、配置、结果和访问权限。
